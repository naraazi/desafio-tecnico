...
Aplicação full-stack para controle de pagamentos e transferências: API REST em Node.js/TypeScript/Express/TypeORM (MySQL) e frontend em Next.js (App Router). Inclui autenticação JWT via cookie HttpOnly, RBAC admin/operator, upload de comprovantes para S3, filtros com paginação/ordenação, relatório por período e testes automatizados.
...
Pré-requisitos: Node.js 20+, npm, MySQL em execução, credenciais AWS.
...
Para criar usuário admin/operator:
...
## Implementações futuras ("O que você implementaria se tivesse mais tempo?")

- Soft delete + auditoria: campos `deletedAt`, trilha de tracking do CRUD e logs estruturados.
- Melhor profundidade nos testes:
  1. Adicionar casos de validação (schemas celebrate) para inputs inválidos;
  2. Cobrir caminhos de upload com tipos/limites de arquivo;
  3. Testes de rota/integrados com supertest + sqlite em memória;
  4. Cobertura de middleware `errorHandler`.
- UX avançada: loading/skeletons, toasts de sucesso/erro, otimistic updates e indicador de progresso no upload.
- Observabilidade: métricas (Prometheus), tracing (OpenTelemetry) e errortracking (Sentry) na API e no frontend.
- Qualidade/infra: CI/CD com lint/test/build, migrations TypeORM no lugar de syncronize, healthchecks e rate limiting.
- Relatórios baixáveis: opção de download de relatório em CSV/PDF com visualizador do comprovante em mídia.
